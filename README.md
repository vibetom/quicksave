...
| Editor | `agents/editor.py` | **Two gates.** (1) A mechanical originality check: a 10-word run shared with a source, or high n-gram overlap, is a rejection with the offending passages quoted back. (2) An LLM standards review: hallucinated facts, rumor-as-fact, over-promising headlines, or disrespectful tone fail the draft. The editor returns specific feedback and the writer **revises the draft** (keeping its angle) until approved or the revision cap (`cadence.max_revisions`) is hit — exactly like a desk sending copy back. If the editor judges the *premise* itself unpublishable (no real news, pure rumor, inherently disrespectful), it flags the draft `fatal` and the story is spiked immediately without burning revisions. |
